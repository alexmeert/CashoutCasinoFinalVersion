...
public partial class NetID : MultiplayerSynchronizer
{
	...
	[Export] public MultiplayerSpawner _myNetworkCore;
	...
	private void Initialize(long peerIdOwner)
	{
		...
		if (peerIdOwner == 1)
			IsServer = true;
		// IsLocal = this object is owned by ME (my multiplayer peer ID)
		IsLocal = (OwnerId == Multiplayer.GetUniqueId());
	}
	...
	~NetID()
	{
		...
		if (Multiplayer.IsServer())
		{
			...
			((NetworkCore)_myNetworkCore).NetDestroyObject(this);
		}
	}
