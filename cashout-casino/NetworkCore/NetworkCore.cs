...
public partial class NetworkCore : MultiplayerSpawner
{
	...
	public Node NetCreateObject(int index, Vector3 initialPosition, Quaternion rotation, long owner = -1L)
	{
		...
		// For nodes without NetID, set authority after AddChild.
		// The owning peer is notified via ClaimAuthority RPC from GameMaster after spawn replicates.
		if (!hadNetId && owner > 0)
		{
			node.SetMultiplayerAuthority((int)owner);
			GD.Print($"[NetworkCore] Set authority of {node.Name} to peer {owner}");
		}
		return node;
	}
